# coding: utf-8
from sqlalchemy import ARRAY, BigInteger, Boolean, CheckConstraint, Column, Date, DateTime, Float, Index, Integer, Numeric, String, Table, Text, text
...
class ExposureDatum(Base):
    ...
    __table_args__ = (
        Index('exposure_date_fips', 'date', 'fips', unique=True),
    )
...
class NewDatum(Base):
    ...
    id = Column(Integer, primary_key=True, server_default=text("nextval('new_data_id_seq'::regclass)"))
